    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Using context, automatically invokes tools to extract entities and relationships from the knowledge graph, initializes simulated individuals, and assigns them unique behaviors and memories based on reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on narrative direction, automatically generates initial activation events and hot topics to guide the initial state of the simulated world
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- 模拟 rounds数配置 - 只有在配置生成完成且 rounds数计算出来后才显示 -->
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For first-time runs, it is strongly recommended to switch to ‘Custom mode’ and reduce the number of simulation rounds for a quick preview and to minimize error risk</p>
                    </div>
                  </div>
                </div>
              </div>
...
// 模拟 rounds数配置
const useCustomRounds = ref(false) // 默认使用自动配置 rounds数
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // 用户Custom rounds数，传递 max_rounds 参数
    params.maxRounds = customMaxRounds.value
    addLog(`开始模拟，Custom rounds数: ${customMaxRounds.value}  rounds`)
  } else {
    // 用户选择保持自动生成的 rounds数，不传递 max_rounds 参数
    addLog(`开始模拟，使用自动配置 rounds数: ${autoGeneratedRounds.value}  rounds`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (res.data.expected_entities_count) {
        ...
        addLog(`从Zep图谱读取到 ${res.data.expected_entities_count}  entities`)
      ...
      addLog(`准备失败: ${res.error || 'Unknown error'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        ...
        addLog(`✗ 准备失败: ${data.error || 'Unknown error'}`)
        stopPolling()
        stopProfilesPolling()
      }
    }
  } catch (err) {
    console.warn(' poll状态失败:', err)
  }
}
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          addLog('正在generating agent profiles配置...')
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.config.time_config) {
          ...
          addLog(`Time config: ${tc.minutes_per_round} min per round, ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} total rounds`)
        }
...
const loadPreparedData = async () => {
  ...
  addLog(`已加载 ${profiles.value.length}  itemsAgent人设`)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // 配置尚未生成，开始 poll
        addLog('配置Generating，开始 pollWaiting...')
        startConfigPolling()
      }
    }
  } catch (err) {
