    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still being processed. We recommend manually refreshing the graph shortly.</span>
...
const showSimulationFinishedHint = ref(false) // Post-simulation finished hint
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    // Calculate curvature: spread for multiple edges, straight for single
    let curvature = 0
    if (totalCount > 1) {
      ...
      // Ensures all edges distribute in same reference frame without overlapping
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      ...
      // Base distance 150, increase 40 per additional edge
    ...
    // Add central gravity to cluster isolated node groups to center
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from right side of node, loop around back
    ...
    // Perpendicular offset, calculated by distance ratio for visible curves
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if actual dragging started (exceeded threshold)
