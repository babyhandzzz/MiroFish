...
document content, and graph information, achieving full automation without manual parameter setting.
...
class TimeSimulationConfig:
    ...
    total_simulation_hours: int = 72  # Default 72 hours simulation (3 days)
    ...
    off_peak_activity_multiplier: float = 0.05  # Extremely low activity during early morning
...
class PlatformConfig:
    ...
    # Viral spread threshold (number of interactions to trigger spread)
...
- Almost no activity during midnight 0-5 AM (activity coefficient 0.05)
- Gradually active in the morning 6-8 AM (activity coefficient 0.4)
- Moderately active during work hours 9-18 (activity coefficient 0.7)
- Peak period during evening 19-22 (activity coefficient 1.5)
- Activity declining after 23:00 (activity coefficient 0.5)
...
- **Important**: The following example values are for reference only. You need to adjust specific time periods based on event nature and participant characteristics
  ...
  - Example: Breaking news may cause discussions even at night, off_peak_hours can be shortened
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        ...
        return TimeSimulationConfig(
            ...
            off_peak_activity_multiplier=0.05,  # Almost no one during early morning
...
- **Time follows Chinese daily routines**: Almost no activity during midnight 0-5, most active during evening 19-22
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<active hour list, considering Chinese daily routines>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configurations must follow Chinese daily routines."
