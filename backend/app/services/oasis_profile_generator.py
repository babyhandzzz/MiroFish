...
class OasisProfileGenerator:
    ...
    # Group/institutional type entities (need representative persona generation)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep does not have a built-in hybrid search interface, so we search edges and nodes
        separately and then merge the results. Uses parallel requests for efficiency.
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # Not setting max_tokens, letting the LLM generate freely
                )
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if the last quote is not followed by a comma or closing bracket, string may be truncated
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for public opinion simulation, maximally restoring existing real-world conditions. You must return valid JSON format. All string values must not contain unescaped newline characters. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, maximally restoring existing real-world conditions.
...
2. persona: Detailed persona description (2000-word plain text), must include:
   ...
   - Personal memory (important part of the persona, describe this individual's connection to events and their existing actions and reactions in events)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for this institution/group entity, maximally restoring existing real-world conditions.
...
2. persona: Detailed account profile description (2000-word plain text), must include:
   - Organization basic info (official name, organizational nature, founding background, main functions)
   ...
   - Institutional memory (important part of the persona, describe this institution's connection to events and their existing actions and reactions in events)
...
- Institutional account speech must match its identity positioning"""
    ...
    # Preserve old method name as alias for backward compatibility
