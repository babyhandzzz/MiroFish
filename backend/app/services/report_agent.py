...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Log section content generation complete (content only, does not mean section is done)"""
        self.log(
...
- Need rich material supporting report sections

[Returns]
- Relevant fact originals (can be directly cited)
- Core entity insights
- Relationship chain analysis"""

TOOL_DESC_PANORAMA_SEARCH = """\
[Breadth Search - Full Panorama View]
This tool gets the complete picture of simulation results, especially suitable for understanding event evolution. It will:
...
This is not LLM simulation, but calls the real interview interface to get original responses from simulated Agents.
...
[Use Cases]
- Need perspectives from different roles on an event (What do students think? Media? Officials?)
...
DO focus on "what the future would look like" — simulation results ARE the predicted future
...
2. [Must cite Agent original speech and actions]
   - Agent speech and behavior are predictions of future human behavior
   - Use citation format in the report to display these predictions, e.g.:
...
3. [Language consistency - cited content must match report language]
   ...
   - When citing tool-returned content, ensure it flows naturally in the report language
...
4. [Faithfully present prediction results]
   ...
   - If information is insufficient on some aspect, state it honestly
...
- Do NOT add the section main title at the beginning
- Section titles are automatically added by the system, you only write body content
- Use **bold**, paragraph breaks, citations, lists to organize content, but no headings
...
As the first scene of public opinion, the platform served a core function for initial information dissemination:
...
[Tool Usage Suggestions - Mix different tools, don't just use one]
...
4. [Citation format - must be standalone paragraphs]
   Citations must be independent paragraphs with blank lines before and after, not mixed into paragraphs:
...
7. [Emphasis] Do not add any headings! Use **bold** instead of sub-section titles"""
...
3. Mix different tools, don't just use one
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Notice] You have only called {tool_calls_count} tools, at least {min_tool_calls} required. "
...
class ReportAgent:
    ...
    def _generate_section_react(
        ...
        conflict_retries = 0  # Consecutive conflict count of tool call and Final Answer appearing together
        used_tools = set()  # Track tools that have been called
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returned None, break loop and enter forced finalization
                break
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Starting line number to read from (for incremental fetch, 0 means from beginning)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        1. Remove Markdown heading lines at content start that duplicate the section title
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if it's a heading duplicating the section title (skip duplicates in first 5 lines)
                if i < 5:
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Skip separator line immediately following heading
                i += 1
                continue
