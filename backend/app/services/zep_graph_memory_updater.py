...
class AgentActivity:
    ...
    def _describe_create_comment(self) -> str:
        """Create comment - includes comment content and the commented post info"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    Monitors simulation action log files and dynamically updates new agent activities to the Zep graph.
    Groups by platform, sending batches to Zep after accumulating BATCH_SIZE activities.
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args contains full context information (such as original post content, usernames, etc.).
    ...
    def _send_single_text(self, text: str, platform: str, activity_count: int):
        """Send a single text chunk to Zep with retry logic"""
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                self._total_sent += 1
                self._total_items_sent += activity_count
                display_name = self._get_platform_display_name(platform)
                logger.info(f"Successfully sent chunk ({len(text)} chars) for {display_name} to graph {self.graph_id}")
                return
            except Exception as e:
                if attempt < self.MAX_RETRIES - 1:
                    logger.warning(f"Chunk send to Zep failed (attempt {attempt + 1}/{self.MAX_RETRIES}): {e}")
                    time.sleep(self.RETRY_DELAY * (attempt + 1))
                else:
                    logger.error(f"Chunk send to Zep failed after {self.MAX_RETRIES} retries: {e}")
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        ...
        # Safety: if combined text exceeds Zep's limit, split and send in smaller chunks
        ZEP_MAX_CHARS = 9000
        if len(combined_text) > ZEP_MAX_CHARS:
            logger.info(f"Combined text ({len(combined_text)} chars) exceeds limit, splitting into smaller chunks")
            chunk_texts = []
            current_chunk = []
            current_len = 0
            for text in episode_texts:
                if current_len + len(text) + 1 > ZEP_MAX_CHARS and current_chunk:
                    chunk_texts.append("\n".join(current_chunk))
                    current_chunk = []
                    current_len = 0
            ...
            for chunk in chunk_texts:
                self._send_single_text(chunk, platform, len(activities))
                time.sleep(self.SEND_INTERVAL)
            return
    ...
    def _flush_remaining(self):
        ...
        # Then send remaining activities in each platform buffer, chunked by BATCH_SIZE
        # to avoid exceeding Zep's 10,000 character limit per request
        with self._buffer_lock:
            for platform, buffer in self._platform_buffers.items():
                if buffer:
                    ...
                    for i in range(0, len(buffer), self.BATCH_SIZE):
                        ...
                        if i + self.BATCH_SIZE < len(buffer):
                            time.sleep(self.SEND_INTERVAL)
            # Clear all buffers
