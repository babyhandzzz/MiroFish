...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - preparing: If config_generated=True, it means already done
...
def prepare_simulation():
    ...
    Steps:
    1. Check if preparation work already exists
...
def get_prepare_status():
    ...
    try:
        ...
        if not task:
            # Task doesn't exist, but if simulation_id is available, check if preparation is already complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Traverses the reports directory and finds reports matching the simulation_id.
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if simulation is running or completed, it will be stopped first and run logs cleaned up
...
def get_agent_stats(simulation_id: str):
    ...
    Used for frontend Agent activity ranking, action distribution display, etc.
    """
    try:
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be in running state (entered command-waiting mode after completing simulation loop)
...
def close_simulation_env():
    ...
    Sends a close environment command to the simulation, making it gracefully exit command-waiting mode.
